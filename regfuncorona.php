...
{ 
     header('Location: registration.php?errorMessage= Incorrect Captcha !!!'); 
}
else
{ 
    ...
    //{
        $name = $fname." ".$lname;
        $sql    = "INSERT INTO `registration_order` (
        `fname` ,
        `lname` ,
        `email` ,
        `phone` ,
        ...
        VALUES (
        '".addslashes($fname)."', '".addslashes($lname)."',  '".addslashes($bill_email)."', '".$bill_phone."','".$register_date."')";				  
        //echo $sql; die;
        ...
        header('Location: registration.php?errorMessage=Thank you for Registering with Savanifarms&flag=0');  
    //}
	//else
	//{
