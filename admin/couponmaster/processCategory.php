...
{
     $txtaddid       		= addslashes($_POST['txtaddid']);
    $Desc     		= addslashes($_POST['desc']);
	$fromdate    		= addslashes($_POST['fromdate']);
	$todate    		= addslashes($_POST['todate']);
	$minqty    		= addslashes($_POST['minqty']);
	$discperc    		= addslashes($_POST['discperc']);
	$discamt    		= addslashes($_POST['discamt']);
	$couponcode= addslashes($_POST['txtName']);
			$sql9  = "SELECT Coupon_code 
	    	     	  FROM couponmaster 
				 	  WHERE Coupon_code = '$txtaddid'";
			$result9 = dbQuery($sql9);
	if (dbNumRows($result9) == 1)
	{
	    	header('Location: index.php?view=add&error=' . urlencode('Coupon Code already taken. Choose another one'));	
	} 
	else
	{
	    	 $sql = "INSERT INTO couponmaster(`Coupon_code`,`Desc`,`From_Date`,`To_Date`,`Min_Qty`,`Disc_perc`,`Disc_Amt`) 
                     VALUES ('$txtaddid','$Desc','$fromdate','$todate','$minqty','$discperc','$discamt')";
			 echo $sql;
...
function modifyCategory()
{
	...
	$couponcode= addslashes($_POST['txtName']);
		    				...
		    				$sql    = "UPDATE couponmaster  
    	           				  SET `Coupon_code`='$txtaddid',`Desc`='$Desc',`From_Date`='$fromdate',`To_Date`='$todate', 							                                  `Min_Qty`='$minqty',`Disc_perc`='$discperc',`Disc_Amt`='$discamt' WHERE `Coupon_code` = '$couponcode'";
        				//echo $sql;die;
