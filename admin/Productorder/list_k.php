...
$sql = "SELECT *
		...
		WHERE oi.prod_id = p.PordId and o.Order_Id = oi.Order_Id AND o.Order_Date BETWEEN '$appdate' AND '$appdate1' $sqlorder $sqlprod
		GROUP BY o.Order_Id
...
$pagingLink='';
$orderStatus = array('PENDING', 'EXECUTED', 'ALL');
$orderOption = '';
...
<center> Order </center>
 ...
 <tr align="center">
  <td align="right" class="hdbg" colspan="">SELECT ORDER TYPE<select name="cboOrderType" class="box" id="cboOrderType"   onChange="viewProduct1forlist();" >
    <option value="ALL" <? if($catId=='ALL') { ?> selected<? } ?>>All</option>
		<option value="USD" <? if($catId=='USD') { ?> selected<? } ?>>USD</option>
        <option value="INR" <? if($catId=='INR') { ?> selected<? } ?>>INR</option>
  </select></td>
  </tr>
  ...
  To Date : <input name="appdate1" type="text" id="appdate1" title="CLICK HERE TO SELECT THE DATE" onClick="ds_sh(this,'no','','')" size="10" maxlength="10" <? if(isset($appdate1)) {?> value="<? echo $appdate1; ?>"<? } else { ?> value="<? echo date("m-d-Y"); ?>"
   <? } ?>  readonly="yes" ><? include('calreviewreport.php');?>
                ...
                <tr align="center">
  <td align="right" class="hdbg" colspan="">Order Status<select name="cboOrderStatus" class="box" id="cboOrderStatus" <? /* onChange="viewOrder();" */ ?>>
  ...
  <option value="ALL" <? if($product == 'ALL') { ?> selected <? } ?>>ALL</option>
    <? 
	...
	{
	?>
	<option value="<? echo $rowproduct['PordId'];?>" <? if($product == $rowproduct['PordId']) { ?> selected <? } ?>>
	<? echo $rowproduct['PordId']."-".$rowproduct['ProdHead']."-".$rowproduct['ProdName']; ?>
    </option>    
    <?
	}
	 ?>
...
?>
  ...
  <tr>
    ...
    <td align="left"><font color="#000000"><? echo $mn." ".$dy.",".$yr; ?></font></td>
  </tr>
  <tr>
    <td align="right">Customer Name :</td>
    <td align="left"><font color="#000000"><? echo $name ?></font></td>
  </tr>
</table>
...
?>
     ...
     <tr><td align="left">
		  <b>Billing  :</b></td><td  align="left"><b>Shipping  : </b></td><tr>
