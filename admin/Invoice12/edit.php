...
VALUES (
'$catId', '$p1', '$p2', '$p3', '$sid',1,0,NOW(),$invno
)";
...
$sql = "SELECT  PordId,ProdSKU,oi.ProdDesc, oi.Prodprice , oi.qty, oi.backorder, oi.srno,oi.taxperc 
...
<input name="appdate" type="text" id="appdate" title="CLICK HERE TO SELECT THE DATE" onClick="ds_sh(this,'no','','')" onchange="setme();" size="10" maxlength="10" <? if($rowno['CustId'] == '') { ?> <? if(isset($k)) { ?> value="<? echo $k; ?>" <? } else { ?>value="<? echo date("m-d-Y"); ?>" <? } ?> <? } else { ?> value="<? echo $rowno['CustId']; ?>" <? } ?>
     readonly="yes" ><? include('calreviewreportedit.php');?> 
							...
							$resultforno=mysql_query($sqlforno);
							$rowno=mysql_fetch_assoc($resultforno);
								if(isset($catId)) { echo $rowno['invoiceno']; } }
							 ?>
  	...
  	for($i = ($srno - 1); $i <= $k;$i++)
{
	...
	$qty = $_POST[$v];
	$price =  $_POST[$v1];
	$sql="UPDATE invoice SET qty = $qty,backorder= $backorder,Prodprice = $price,Proddesc= '$proddesc',invdate = '$appdate' where CustId = $catId AND invoiceno = $invoiceno AND srno = $i";
	$result=mysql_query($sql);
	}
	else
	{
  	...
  	for($i = ($srno - 1); $i <= $k;$i++)
{
	...
	$sqlfinalsearch="select * from invoice where invoiceno = $invoiceno AND srno = $i AND Prodid != 1";
	$resultfinalsearch=mysql_query($sqlfinalsearch);
	if(mysql_num_rows($resultfinalsearch) == 1)
	{
	...
	$qty = $_POST[$v];
	$price =  $_POST[$v1];
	$sql="UPDATE invoice SET qty = $qty,backorder=$backorder,Prodprice = $price,Proddesc= '$proddesc',invdate = '$appdate' where invoiceno = $invoiceno AND srno = $i";
...
?>
    ...
    <tr class="content"> 
		...
		<input type="text" <? if($ProdSKU != 1) { ?> value="<? echo $backorder; ?>" <? } ?> name="backorder<? echo $srno; ?>" maxsizze="3" size="3" id="backorder<? echo $srno; ?>" <? /*
          onblur="enterqty(this,'<? echo $srno; ?>','<? echo $catId;?>','<? echo $ProdSKU; ?>');" */ ?> style="text-align:right"/><? echo $srno; ?></td>
        <td align="right" valign="top"><input type="text" value="<?php echo $Prodprice; ?>" name="ppricetext<? echo $srno; ?>" id="ppricetext<? echo $srno; ?>" maxsizze="3" size="3" <? /* onBlur="enterqty1(this,'<? echo $srno; ?>','<? echo $catId;?>','<? echo $ProdSKU; ?>');" */ ?> style="text-align:right"/>          &nbsp;</td>
     ...
     <? /* onBlur="enterqty2(this,'<? echo $srno; ?>','<? echo $catId;?>','<? echo $ProdSKU; ?>','<? echo $sid;?>');" */ ?> style="text-align:right"></td>
      <td align="right" valign="top"><input type="text" name="taxamttext<? echo $srno; ?>" id="taxamttext<? echo $srno; ?>" value="<?php echo (((($qty * $Prodprice*$taxperc)/100)+($qty * $Prodprice))) ?>" maxsizze="3" size="3" style="text-align:right"></td>
             <td><input type="submit" name="update1" value="EDIT" maxlength="4"></td>
...
?>
    ...
    Invoice For :
            ...
            </select> Customer Service Contact :
           
             <input type="text" name="continfo" id="continfo">
    </td>
    </tr>
