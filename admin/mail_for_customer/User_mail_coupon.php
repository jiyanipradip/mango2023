...
{	
	...
	location.href="User_mail_coupon.php?from_date="+from_date+"&to_date="+to_date;
}
...
<form name="form1" action="SaveToExcel.php" method="post" target="_blank" onsubmit='$("#datatodisplay").val( $("<div>").append( $("#ReportTable").eq(0).clone() ).html() )'>
<table width="100%" border="1" cellspacing="5" cellpadding="5" class="reference" align="center">
  <tr>
  	<td align="center" bgcolor="#f0f0f0"><strong>SAVANIFARM SEND MAIL REPORT</strong></td>
  </tr>
  <tr>
  	<td align="center">
...
{
	$sql_orderdata ="select * from orderdata where invoiceno BETWEEN '$from_date' AND '$to_date' and Coupon_code!='' ORDER BY Order_Id";
...
?>
<div align="left">
<pre title="Export to Excel">Export to excel<input type="image" src="excel_icon.gif" width="30" height="30" border="0"  ></pre>
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" class="reference" id="ReportTable">
  <tr>
    ...
    <td bgcolor="#f0f0f0">Coupon Code </td>
    ...
    <td bgcolor="#f0f0f0">Shipping Method</td>
  ...
  while($datanew=mysql_fetch_assoc($res_orderdata))
  {
	...
	?>
		...
		$sql_shipm = "SELECT * From shipmethod WHERE METHODID='$data_rr[Ship_Method]'";
		$res_shipm = mysql_query($sql_shipm);
		$num_shipm=mysql_num_rows($res_shipm);
		$data_shipm =mysql_fetch_assoc($res_shipm);
 ...
 ?>        
          <tr>
            <td><? echo $invoice_number; ?>
            <?php /*<p><a href="javascript:setprocessflag('send_mail_demo.php?invoice_number=<? echo $invoice_number; ?>','window1');">Send Email</a></p> */?></td>
            <td><? echo $ordate; ?></td>
            ...
            <td><? echo $ship_zip; ?></td>
            <td><? echo $customer_type; ?></td>
             <td><? echo strtoupper($data_shipm['METHOD']); ?></td>
            <td><? //echo $ProdName; ?>
