...
//echo $barcode2;
							...
							$sqlforno="select * from creditmemomaster order by memono DESC";
...
}else
{
	...
	if(mysql_num_rows($result) == 1)
	{
		...
		$p4 = $rowprod['TaxCode'];
			...
			$sid = session_id();
		$sqlduplicatecheck="select * from creditmemo where CustId = '$catId'AND Prodid = '$p1' AND Proddesc = '$p2' AND Prodprice = '$p3' AND sessid = '$sid'";
		$resultduplicatecheck=dbQuery($sqlduplicatecheck);
		//if(mysql_num_rows($resultduplicatecheck) == 0)
		
		
		$sqlinvno="select memono from creditmemomaster ORDER BY memono DESC";
		$resultinvno=mysql_query($sqlinvno);
		$rowinvno=mysql_fetch_assoc($resultinvno);
		$memono=$rowinvno['memono'];
			$sqlforinsert="INSERT INTO `dentadepot`.`creditmemo` (
`CustId` ,
`Prodid` ,
`Proddesc` ,
`Prodprice` ,
...
if(isset($generate))
{
$sqlgeneratesrno="insert into creditmemomaster(`sid`) VALUES ('sid')";
...
$sql = "SELECT ProdName,PordId,ProdSKU,p.ProdDesc, oi.Prodprice , oi.qty,oi.srno,oi.taxperc 
	    FROM creditmemo  oi, productmast p 
		WHERE oi.Prodid = p.PordId and oi.Prodid !=1 and oi.CustId = $catId and oi.sessid='$sid' and oi.memono = $memono
...
//	$sql2 = " AND  p.CatagoryId = '$catId'";
	//$queryString = "catId='$catId'";
		$sid = session_id();
		$sql2="select * from creditmemo where sessid = '$sid' AND Prodid = '1' AND CustId = '$catId'";
		$result2=mysql_query($sql2);
		if(mysql_num_rows($result2) == 0)
		{
		$sql1="insert into creditmemo(`sessid`,`Prodid`,`CustId`) values('$sid',1,'$catId')";
		//echo $sql1;
...
<input name="appdate" type="text" id="appdate" title="CLICK HERE TO SELECT THE DATE" onClick="ds_sh(this,'no','','')" onchange="setme();" size="10" maxlength="10" <? if($rowno['CustId'] == '') { ?> <? if(isset($k)) { ?> value="<? echo $k; ?>" <? } else { ?>value="<? echo date("Y-m-d"); ?>" <? } ?> <? } else { ?> value="<? echo $rowno['CustId']; ?>" <? } ?>
     readonly="yes" ><? include('calreviewreportedit.php');?> 
							...
							<?	if(isset($catId)) { echo $rowno['memono']; }
							 ?>
...
<tr id="infoTableHeader"> 
    ...
    <tr id="infoTableHeader"> 
        <td colspan="9" class="hdbg"><form method="post" name="formbarcode" action="">Enter Barcode<input type="text" name="barcodeentry" id="barcodeentry"><input type="submit" name="go" value="GO"></form></td>
    </tr>
  	...
  	for($i = ($srno - 1); $i <= $k;$i++)
{
	...
	$selfordate="select * from creditmemomaster  where memono = $memono";
	...
	$sqlfinalsearch="select * from creditmemo where CustId = $catId AND memono = $memono AND srno = $i";
	...
	$qty = $_POST[$v];
	$price =  $_POST[$v1];
	$sql="UPDATE creditmemo SET qty = $qty,Prodprice = $price,invdate = '$appdate' where CustId = $catId AND memono = $memono AND srno = $i";
	//echo $sql."<br>";
...
$sql="UPDATE creditmemomaster  SET invfor = '$supply',contact = '$continfo' where memono = $memono";
	//echo $sql."<br>";die;
	$result=mysql_query($sql);
...
<?
if(isset($sid)){
$sql = "select memono from creditmemo where custid='$catId' AND sessid='$sid'";
$result=mysql_query($sql);
