...
if((isset($_POST['barcodeentry'])) || (isset($barcode2)))
 {
	...
	if(mysql_num_rows($result) == 1)
		...
		$sqlinvno="select memono from creditmemomaster where memono = $memono";
  	...
  	for($i = ($srno - 1); $i <= $k;$i++)
{
	...
	$selfordate="select * from creditmemomaster where invoiceno = $memono";
...
?>
    ...
    <?
if(isset($memono)){
$sqlyy = "select memono from creditmemo where custid='$catId' AND memono='$memono'";
$resultyy=mysql_query($sqlyy);
$popat=mysql_fetch_assoc($resultyy);
?>
<? $k = $popat['memono'];
	$sqlzz="select * from creditmemomaster where memono = $memono";
	//echo $sqlzz;
  ...
  <? /*  <a href="#" onclick="javascript:window.open('modify/print.php?oid=<? echo $catId; ?>&amp;sis=<? echo $sid; ?>&amp;invoiceno=<? echo $sirno; ?>&suply=<? echo $rows['invfor'] ?>&contact=<? if(isset($sid)) { echo $rows['contact']; }?>', '_blank');">           PRINT            </a> */?>
     <?
if(isset($memono)){
$sqlyy = "select * from creditmemo where custid='$catId' AND memono='$memono'";
$resultyy=mysql_query($sqlyy);
$popat=mysql_fetch_assoc($resultyy);
