...
<form action="processCategory.php?action=modifysubcategory&catId=<?php echo $catId; ?>&scat=<? echo $scatid; ?>" method="post" enctype="multipart/form-data" name="frmCategory" id="frmCategory">
 ...
 <tr><td width="150" class"label">CATEGORY</td><td class="content"><select name="cboCategory" class="box" id="cboCategory" onChange="viewProduct();">
     <option value='' selected>All Category</option>
	<option value='<?php echo stripslashes($categoryList); if(stripslashes($categoryList)==$catId){ echo "selected";}?>'><?php echo stripslashes($categoryList); ?></option>
   </select></td>
  </tr>
  <tr> 
