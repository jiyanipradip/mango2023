...
{
$sql = "SELECT DISTINCT(memono),invdate from debitmemo where CustId = $catId AND SUBSTRING(invdate,1,10)
BETWEEN '$appdate' AND '$appdate1' AND Prodid != '1'
 ORDER BY srno DESC";
}
else if((isset($catId) && ($catId = '0') || ($catId = 'undefined')) && isset($appdate) && isset($appdate1))
{
	$sql = "SELECT DISTINCT(memono),invdate from debitmemo where SUBSTRING(invdate,1,10)
BETWEEN '$appdate' AND '$appdate1' AND Prodid != '1'
 ORDER BY srno DESC";
}
else
{
	$sql = "SELECT DISTINCT(memono) from debitmemo where memono != 0 AND Prodid != '1' AND CustId != ''
...
if (dbNumRows($result) > 0) {
	...
	$memono = $row['memono'];
	$sqlnow="select * from debitmemo where memono='$memono' AND Prodid != '1'";
		$resultnow=mysql_query($sqlnow);
		$rownow=mysql_fetch_assoc($resultnow);
		$CustId = $rownow['CustId'];
		$sql1="select * from vendormast where vendorid='$CustId'";
...
;
   ...
   $sqlforamount="select * from debitmemo where CustId = '$CustId' and memono = '$memono' AND memono != '0'";
